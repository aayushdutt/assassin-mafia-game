...
router.get(
  ...
  (req, res) => {
    ...
    User.findById(userId, (err, foundUser) => {
      ...
      if (!foundUser.canPlay || foundUser.role !== "villager") {
        return res.send("Vote was not allowed");
      }

      const increment = foundUser.isKilled ? 1 : 2;

      User.findByIdAndUpdate(
        req.params.participantId,
        { $inc: { votesRecieved: increment } },
        (err, updatedParticipant) => {
...
router.get(
  ...
  (req, res) => {
    ...
    User.findById(userId, (err, foundUser) => {
      if (!foundUser || err)
        return res.send("Couldn't assassinate, please try again");
  ...
  (req, res) => {
    ...
    User.findById(userId, (err, foundUser) => {
      ...
      User.findByIdAndUpdate(
        ...
        (err, updatedParticipant) => {
          if (err)
            return res.send("Couldn't assassinate, internal server error");
          User.findByIdAndUpdate(userId, { canPlay: false }, (err) => {
            if (err)
              return res.send("Couldn't assassinate, internal server error");
            return res.redirect("/room");
