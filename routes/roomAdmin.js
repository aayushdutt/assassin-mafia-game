...
router.post("/assignRoles", authenticateAdmin, (req, res) => {
  ...
  User.find({ roomId: roomId }).then((participants) => {
    ...
    let remAssassins = parseInt(assassins),
      remSeers = parseInt(seers);
    for (let i = 0; i < len; i++) {
    ...
    Promise.all(queue)
      .then(() => {
        Room.findByIdAndUpdate(
          roomId,
          { rolesAssigned: true, status: ["Roles are assigned"] },
          (err, updatedRoom) => {
...
router.get("/startRound", authenticateAdmin, (req, res) => {
  ...
  Room.findById(roomId, (err, foundRoom) => {
    if (err || !foundRoom || !foundRoom.rolesAssigned) return;

    User.updateMany(
      { roomId, isKilled: false },
      {
        votesRecieved: 0,
        canPlay: true,
      },
  ...
  User.find({ roomId }, (err, participants) => {
    ...
    participants.forEach((participant, idx) => {
      if (!participant.isKilled) {
        if (participant.isAssassinated) {
          ...
          participant.canPlay = participant.role === "villager"; // villagers can still play
    ...
    if (maxVotes > 0 && maxVoted) {
      if (!maxVoted.isKilled) {
        ...
        maxVoted.canPlay = maxVoted.role === "villager";
